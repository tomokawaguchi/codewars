...
object EqualSidesOfAnArray {
  fun findEvenIndex(arr:IntArray):Int {
    ...
    while(rightSum != leftSum && i < arr.size-1){
        rightSum -= arr[i+1]
        leftSum += arr[i]
        i++
    }
    return if(rightSum != leftSum) -1 else i   
  }
}
...
object EqualSidesOfAnArray {
    ...
    // indexOfFirst() --> returning the first matching constion
...
object EqualSidesOfAnArray {
  fun findEvenIndex(arr:IntArray):Int {
    for (i in arr.indices) {
        // sliceArray() --> return the array containg the specified indices
